...
3. Freshness/provenance checks.
   Delivery outputs now write lightweight fingerprints so existing outputs can be
   classified by whether they match the current sources, settings, and code,
   rather than only by whether a destination file exists.
