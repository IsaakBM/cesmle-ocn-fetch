...
echo "[info] Experiment: $EXPT (historical)"
...
for var in "${VARS[@]}"; do
  ...
  # Strict, var-specific regex to avoid unwanted siblings; uses historical experiment tag
  case "$var" in
    TEMP|SALT)
      REGEX="b\.e11\.${EXPT}\.f09_g16\.${MEM_RE}\.pop\.h\.${var}\.${DATE_RE}\.nc"
      ;;
    UVEL)
      REGEX="b\.e11\.${EXPT}\.f09_g16\.${MEM_RE}\.pop\.h\.UVEL\.${DATE_RE}\.nc"   # exclude UVEL2
      ;;
    O2)
      REGEX="b\.e11\.${EXPT}\.f09_g16\.${MEM_RE}\.pop\.h\.O2\.${DATE_RE}\.nc"     # exclude O2SAT, O2_CONSUMPTION, etc.
      ;;
    *)
  ...
  if [[ "${#files[@]}" -gt 0 ]]; then
    ...
    # Fallback: probe known spans for every member (historical)
    echo "      [info] no list entries; probing historical spans…"
    for mem in $MEMBERS; do
      for span in "${FALLBACK_SPANS[@]}"; do
        fname="b.e11.${EXPT}.f09_g16.${mem}.pop.h.${var}.${span}.nc"
        url="${INDEX_URL}${fname}"
        dest="${OUTDIR}/${fname}"
        if [[ -f "$dest" ]]; then
          echo "      [skip] exists: $fname"
