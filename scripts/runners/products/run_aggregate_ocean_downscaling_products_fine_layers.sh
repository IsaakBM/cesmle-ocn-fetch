...
for subtree in "${SUBTREES[@]}"; do
  ...
  jid=$(
    sbatch --parsable \
      ...
      --export=ALL,BIN_SET=fine,IN_ROOT="${subtree}",OUT_ROOT="${out_subtree}",OVERWRITE="${OVERWRITE}",FUTURE_MODELS="${FUTURE_MODELS}",EXCLUDE_FUTURE_MODELS="${EXCLUDE_FUTURE_MODELS}" \
      "${TOOL_SCRIPT}"
  )
