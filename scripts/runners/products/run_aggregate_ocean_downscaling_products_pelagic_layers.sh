...
echo "Submitting curated ocean product pelagic-layer aggregation jobs by subtree:"
...
for subtree in "${SUBTREES[@]}"; do
  ...
  jid=$(
    sbatch --parsable \
      --job-name="pel_${job_tag}" \
      --output="${LOG_DIR}/pelagic_layers_${job_tag}_%j.out" \
      --error="${LOG_DIR}/pelagic_layers_${job_tag}_%j.err" \
      --export=ALL,BIN_SET=pelagic,IN_ROOT="${subtree}",OUT_ROOT="${out_subtree}",OVERWRITE="${OVERWRITE}" \
      "${TOOL_SCRIPT}"
  )
