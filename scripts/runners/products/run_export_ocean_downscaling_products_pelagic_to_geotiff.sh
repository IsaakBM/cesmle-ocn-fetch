...
for subtree in "${SUBTREES[@]}"; do
  ...
  jid=$(
    sbatch --parsable \
      ...
      --cpus-per-task=5 \
      --export=ALL,IN_ROOT="${subtree}",OUT_ROOT="${out_subtree}",TMP_DIR="${tmp_subtree}",OVERWRITE="${OVERWRITE}",RESOLUTIONS="${RESOLUTIONS}",NPROC=5,FUTURE_MODELS="${FUTURE_MODELS}",EXCLUDE_FUTURE_MODELS="${EXCLUDE_FUTURE_MODELS}" \
      "${TOOL_SCRIPT}"
  )
